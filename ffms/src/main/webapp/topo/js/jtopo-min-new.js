!function(window) {
    ...
    JTopo = {
        ...
        MouseCursor: {
            ...
            closed_hand: "url(../resource/theme/default/images/topo/cur/closedhand.cur) 8 8, default"
        },
...
function(JTopo) {
    ...
    function delFromArr(arr, a){
        ...
        return 0 > a ? arr : arr.slice(0, a).concat(arr.slice(a + 1, arr.length))
    }
    ...
    function toJson(a) {
        var b = "backgroundColor,visible,mode,rotate,alpha,scaleX,scaleY,shadow,translateX,translateY,areaSelect,paintAll".split(",")
          , c = "name,text,elementType,x,y,width,height,visible,alpha,rotate,scaleX,scaleY,fillColor,shadow,transformAble,zIndex,dragable,selected,showSelected,font,fontColor,textPosition,textOffsetX,textOffsetY".split(",")
          , d = "{";
        d += "frames:" + a.frames,
        d += ", scenes:[";
    ...
    function b(a) {
        return {
            ...
            getData: function(b, c) {
                ...
                null  != j && null  != k ? this.setSize(b, c) : this.setSize(150, 100);
    ...
    function c(c) {
        ...
        this.remove = function(a) {
            ...
            for (var b = 0; b < this.childs.length; b++)
                if (this.childs[b] === a)
                    return a.stage = null ,
                    this.childs = Jtopo.util.delFromArr(this.childs, b),
                    this;
            return this
        }
        ,
...
function(a) {
    function b(c) {
        ...
        this.initialize = function() {
            ...
            this.paintAll = !1,
            this.name = "";
            var c = "name,background,backgroundColor,mode,paintAll,areaSelect,translate,translateX,translateY,lastTranslatedX,lastTranslatedY,alpha,visible,scaleX,scaleY".split(",");
        ...
        ,
        // 置顶元素（hzm_add）
        this.stick = function (b) {
            // 分析完源代码，自己添加的置顶方法
            this.zIndexMap["" + b.zIndex] = a.util.removeFromArray(this.zIndexMap["" + b.zIndex], b);
            this.zIndexMap["" + b.zIndex].push(b);
        },
        this.clear = function() {
            var a = this;
            this.childs.forEach(function(b) {
        ...
        this.removeFromSelected = function(a) {
            for (var b = 0; b < this.selectedElements.length; b++) {
                ...
                a === c && (this.selectedElements = Jtopo.util.delFromArr(this.selectedElements, b))
            }
        }
        ,
...
function(a) {
    function b() {
        this.initialize = function() {
            ...
            this.zIndex = 0,
            this.name = "";
            var a = "name,x,y,width,height,visible,alpha,rotate,scaleX,scaleY,strokeColor,fillColor,shadow,shadowColor,shadowOffsetX,shadowOffsetY,transformAble,zIndex".split(",");
    ...
    function d() {
        ...
        this.selectedHandler = function(b) {
            ...
            // hzm_update
            // 只要是我自定义的 containerEx类型，均直接支持editAble拉扯resize功能（原先jtopo的设定还得scene的mode模式设为edit，两个条件同时true才行。。。）
            this.elementType == 'containerEx' ? (this.editAble = !0) : (
    ...
    function b(c) {
        ...
        this.paintAlarmText = function(a) {
            if (null  != this.alarm && "" != this.alarm) {
            	...
            	var alarmContent = [];
            	alarmContent = this.alarm.split('@');
				var maxLength = alarmContent[0].replace(/[^\x00-\xff]/g, 'xx').length;
				var maxStr = alarmContent[0];
				for(var j = 1; j < alarmContent.length; j ++){
					if(maxLength < alarmContent[j].replace(/[^\x00-\xff]/g, 'xx').length){	
						maxLength = alarmContent[j].replace(/[^\x00-\xff]/g, 'xx').length;
						maxStr = alarmContent[j];
					}
				}
            	
                /*var b = this.alarmColor || "255,0,0"*/
            	var b = this.alarmHeadColor || "255,0,0"
                  , c = this.alarmAlpha || 1;
                a.beginPath(),
                a.font = this.alarmFont || "14px 宋体";
                /*var d = a.measureText(this.alarm).width + 6
                  , e = a.measureText("田").width + 6*/
                var d = a.measureText(maxStr).width + 6
                  , e = (a.measureText("田").width + 6) * alarmContent.length
                  , f = this.width / 2 - d / 2
                  , g = -this.height / 2 - e - 8;
                ...
                if(alarmContent.length > 1){
					for(var j = 0; j < alarmContent.length; j ++){
						a.fillText(alarmContent[j], f + 3, g + e - 4 - (a.measureText("田").width + 6) * j);
					}					
				}else{
					a.fillText(this.alarm, f + 3, g + e - 4);
				}
                a.closePath()
            }
        }
        ...
        this.paintText = function(a, b) {
            ...
            this.text && this.text.length > 0) {
                ...
                this.nodeA === this.nodeZ) {
                	...
                	var linkText = [];
                	linkText = this.text.split('@');   				
    				for(var j = 0; j < linkText.length; j ++){
    					a.fillText(linkText[j], e - g / 2, f - h / 2 + j *　20);
    				}
                }
