(function () {
    ...
    TopoCore2.prototype = {
        ...
        renderTopo: function () {
            ...
            // init拓扑
            var topology = document.getElementById('topology2');
        ...
        deleteLink: function () {
            var self = this;
            var currentLinkId = this.currentSelectLink.linkId;
            var currentLinkName = this.currentSelectLink.text;
            this.showConfirm({
                message: "确定删除连线 " + currentLinkName + " ?",
                handle: function () {
                    self.deleteLinkLocalFun(self.currentSelectLink);
                },
                cancel: function () {
        ...
        // 本地删除连线方法
        deleteLinkLocalFun: function (link) {
            var j;
            // 删除连线
            this.scene.remove(link);
            j = this.links.indexOf(link);
            j != -1 && this.links.splice(j, 1);
            // 清空连线占用的左、右两个端口
            this.delOccupyIF(link.aNode, link.aIf);
            this.delOccupyIF(link.zNode, link.zIf);
        },
        ...
        addContainer: function (evt) {
            var self = this;
            this.showDialog({
                winId: 'containerMgrWin',
                title: '添加容器',
                width: '550',
                templateUrl: basePath.VIEW_PATH + "business/itportal/views/topo/containerMgr.html",
                controller: 'topo.containerMgr.ctrl as containerMgr',
                saveCallBack: function () {
                    var field = $('#containerMgrDiv').scope().containerMgr.getData();
                    // 当前位置
                    var pos = self.getRelativeOffset(evt);
                    var containerName = "默认容器";//field.name || '';
                    field.showText == 'OFF' && (containerName = '');
                    var tmpContainer = new JTopo.ContainerEx(containerName);
                    // 文本位置
                    tmpContainer.textPosition = field.textPos;
                    tmpContainer.setBound(pos.left, pos.top, 100, 100);
                    tmpContainer.fillColor = '243,250,255';
                    tmpContainer.borderColor = '191,191,191';
                    tmpContainer.borderWidth = 1;
                    tmpContainer.alpha = 0.6;
                    tmpContainer.fontColor = '255,130,46';
                    tmpContainer.font = '16px 微软雅黑';
                    tmpContainer.editAble = true;
                    tmpContainer.showSelected = false;
                    self.scene.add(tmpContainer);

                    // 关闭弹窗
                    $('#containerMgrWin').widget().destroy();
                }
            })
        },

        // 添加新节点
        addNewNode: function (evt) {
            var self = this;
            self.showDialog({
                winId: 'addNodeWin',
                title: '添加节点',
                width: '550',
                templateUrl: basePath.VIEW_PATH + "business/itportal/views/topo/addNode.html",
                controller: 'topo.addNode.ctrl as addNode',
                saveCallBack: function () {
                    var vaildResult = UnifyValid.FormValid($("#addNodeDiv"));
                    if (!vaildResult) {
                        return;
                    }
                    var data = $("#addNodeDiv").scope().addNode.getData();
                    var deviceType = data.deviceType;
                    var deviceName = data.deviceName;
                    var deviceId = data.deviceId;
                    var offset = self.getRelativeOffset(evt);
                    // 缩放位置我算不来！！！
                    // this.scene.scaleY
                    // this.scene.scaleX
                    var node = {
                        ...
                        nodeIp: '', // 先写个空上去吧
                        manual: 1,
                        y: offset.top - self.scene.translateY/* + evt.screenY + evt.offsetY*/,
                        x: offset.left - self.scene.translateX,
                        icon: self._getIcon(deviceType),
                        nodeType: deviceType
                    };
                    self.renderNode(node);
                    //
                    $('#addNodeWin').widget().destroy();
                }
            });
        },
        ...
        addRelation: function (evt) {
            var self = this;
            self.showDialog({
                winId: 'addRelationWin',
                title: '添加关系',
                width: '750',
                templateUrl: basePath.VIEW_PATH + "business/itportal/views/topo/addRelation.html",
                controller: 'topo.addRelation.ctrl as addRelation',
                saveCallBack: function () {
                    var vaildResult = UnifyValid.FormValid($("#addRelationDiv"));
                    if (!vaildResult) {
                        return;
                    }
                    var deferred = $("#addRelationDiv").scope().addRelation.save();
                    deferred && deferred.then(function (response) {
                        if(response && response.successed){
                            $('#addRelationWin').widget().destroy();
                            // 重刷拓扑
                            self.refreshView();
                            self.showMyToaster('success', '添加成功');
                        }else{
                            self.showMyToaster('error', '添加失败');
                        }
                    });
                }
            });
